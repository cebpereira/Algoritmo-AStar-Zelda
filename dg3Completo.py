...
RECT_WIDTH = round(SCREEN_WIDTH / len(dungeon3[0]))
RECT_HEIGHT = floor(SCREEN_HEIGHT / len(dungeon3))
...
# Define o objetivo
goal = None
for row in range(len(dungeon3)):
    for col in range(len(dungeon3[0])):
        if dungeon3[row][col] == "PingenteD3":
            goal = (col, row)
...
# Chama a função A*
path = a_star(start, goal, dungeon3)
