...
RECT_WIDTH = round(SCREEN_WIDTH / len(dungeon1[0]))
RECT_HEIGHT = floor(SCREEN_HEIGHT / len(dungeon1))
...
# Define o objetivo
goal = None
for row in range(len(dungeon1)):
    for col in range(len(dungeon1[0])):
        if dungeon1[row][col] == "PingenteD1":
            goal = (col, row)
...
# Chama a função A*
path = a_star(start, goal, dungeon1)
